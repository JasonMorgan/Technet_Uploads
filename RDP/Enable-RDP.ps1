...
<#
.SYNOPSYS
Enables RDP on a local or remote workstation
 
.DESCRIPTION
Uses the Win32_TerminalServiceSetting class to enable RDP on a remote computer
 
.EXAMPLE
Enable-RDP -ComputerName TestVM

Enables RDP on the remote workstation TestVM, produces not output by default

.EXAMPLE
Get-Content .\Servers.txt | Enable-RDP -DCOM

Enables RDP on a list of remote servers contained in Services
